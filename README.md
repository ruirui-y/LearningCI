> **v0.4 主线变化**：NebulaRPC 不再按 68 个细粒度节点重复旧能力。现在采用“旧项目快速回顾 → AI 辅助迁移 baseline → 新能力逐项开发 → bRPC/gRPC 验证 → 求职证据”的 14 节点主线。
...
小节 AI 评分不再压成一段文字。Today 页会显示四维分数，并把每个扣分点拆成编号卡片；问题区域可以独立滚动。若问题绑定了叶子任务 ID，可一键跳回对应任务修改源码路径、函数入口或证据备注。

新版小节评分提示词优先要求结构化 `issues`，旧数据库中的 `weaknesses` 仍然兼容。
