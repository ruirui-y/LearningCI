...
不再要求做完 6 个子问题后，再人为写一个“形成审计结论”来重复归纳。

`NRPC-S0-01` 原 41 个任务中有 11 个历史重复收束任务，v0.3.3 在 UI 中不再要求执行，因此实际为 **30 个学习叶子任务 + 5 个小节验收**。

源码路径、函数入口和证据备注现在 **450ms 自动保存到 SQLite**；切换任务、暂停任务、结束学习、关闭程序前都会强制保存一次。
...
- 基础叶子任务：1065 个；
- 每个节点都有首次固定试卷；
- `NRPC-S0-01`：原始执行包 41 项；v0.3.3 去除重复总结后实际执行 30 个叶子任务 + 5 个小节 AI 验收；
